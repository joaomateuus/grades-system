<script setup lang="ts">
import { Aluno } from '../../interfaces/Aluno';
import { onMounted, ref } from 'vue';
import { deleteAlunoService, getAlunosService, updateAlunoService } from '../../services/alunos';

const alunos = ref<Array<Aluno>>([]);
const editing = ref<boolean>(false);

const fetchAlunos = async () => {
    try {
        const { data, errors } = await getAlunosService();
        alunos.value = data as Array<Aluno>;
    } catch (error) {
        console.log(error);
        return alert("Não foi possivel buscar os alunos")
    }
}

const updateAluno = async (aluno: Aluno) => {
    try {
        const { data, errors } = await updateAlunoService(aluno);
        if (!errors) {
            editing.value = false
            fetchAlunos();
            alert("Aluno atualizado com sucesso");
        }
    } catch (error) {
        console.log(error);
        alert("Não foi possivel atualizar aluno")
    }
}

const deleteAluno = async (id: number) => {
    try {
        const { data, errors } = await deleteAlunoService(id);
        if (!errors) {
            fetchAlunos();
            deleteAlunoService(id);
            alert("Aluno deletado com sucesso");
        }
    } catch (error) {
        console.log(error);
        console.log("Não foi possivel deletar aluno")
    }
}

onMounted(() => {
    fetchAlunos();
})

</script>
<template>
    <div class="flex flex-col items-center justify-start w-full h-screen">
        <div class="flex justify-center items-center h-44 w-full">
            <div class="flex items-center justify-between w-full">
                <span class="text-black text-4xl">Alunos</span>

                <div class="flex items-center justify-evenly w-96">
                    <button type="button"
                        class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                        <router-link to="/alunos/create">
                            Adicionar Aluno
                        </router-link>
                    </button>
                </div>
            </div>
        </div>
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg w-4/5">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        <th scope="col" class="px-6 py-3">
                            Nome
                        </th>
                        <th scope="col" class="px-6 py-3">
                            Email
                        </th>
                        <th scope="col" class="px-6 py-3">
                            Data de Nascimento
                        </th>
                        <th scope="col" class="px-6 py-3">
                            Ações
                        </th>
                    </tr>
                </thead>
                <tbody v-for="aluno in alunos">
                    <tr
                        class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                        <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <span v-if="!editing">{{ aluno.nome }}</span>
                            <input class="rounded-md" v-else v-model="aluno.nome" type="text">
                        </td>
                        <td class="px-6 py-4">
                            <span v-if="!editing">{{ aluno.email }}</span>
                            <input class="rounded-md" v-else v-model="aluno.email" type="text">
                        </td>
                        <td class="px-6 py-4">
                            <span v-if="!editing">{{ aluno.data_nascimento }}</span>
                            <input class="rounded-md" v-else v-model="aluno.data_nascimento" type="date">
                        </td>
                        <td v-if="!editing"
                            class="flex w-full items-center justify-evenly font-medium text-blue-600 dark:text-blue-500 hover:underline mt-2 ">
                            <img class="h-4 w-4 m-2" @click="editing = !editing"
                                src="https://cdn-icons-png.flaticon.com/512/1159/1159633.png" alt="">
                            <img @click="deleteAluno(aluno.id)" class="h-4 w-4 m-2"
                                src="https://cdn-icons-png.flaticon.com/512/1799/1799391.png" alt="">
                        </td>
                        <td class="flex justify-evenly w-full px-6 py-4" v-else>
                            <span
                                @click="updateAluno({ id: aluno.id, nome: aluno.nome, email: aluno.email, data_nascimento: aluno.data_nascimento })"
                                class="text-sm">
                                <img class="h-6 w-6" src="https://cdn-icons-png.freepik.com/512/5610/5610944.png"
                                    alt="">
                            </span>
                            <span @click="editing = false" class="text-sm">
                                <img class="h-6 w-6"
                                    src="data:image/png;base64,"
                                    alt="">
                            </span>
                        </td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</template>


<style scoped></style>